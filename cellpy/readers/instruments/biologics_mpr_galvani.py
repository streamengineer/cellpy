...
def _dev_galvani():
    ...
    test_data_path = Path("../../../../cellpy_dev_resources/2021_galvani_data").resolve()
    file_name1 = "bio-logic2.mpr"
    file_path = test_data_path / file_name1
    print(file_path.name)
    print(file_path.is_file())

    mpr_file = BioLogic.MPRfile(str(file_path))
    df = pd.DataFrame(mpr_file.data)
    print(df)
    print(df.columns)
    df.plot(x="time/s", y="Ewe/V")
    # plt.show()
    # This did not work satisfactory - the dataframes galvani creates as default are without current


if __name__ == "__main__":
    print(" biologic loader (galvani) ".center(80, "-"))
    _dev_galvani()
